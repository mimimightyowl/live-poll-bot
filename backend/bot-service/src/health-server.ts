    ...
    try {
      if (req.url === '/health' && req.method === 'GET') {
        ...
        try {
          ...
          await usersClient.getUserByTelegramId({
            telegram_id: 'health_check',
          });
      ...
      logger.error('Health check handler error:', error as Error);

      // Ensure we send a response even if headers were already sent
      if (!res.headersSent) {
